...
namespace MIS_Backend.Controllers
{
    ...
    public class PatientController : ControllerBase
    {
        ...
        [HttpGet]
        [Authorize]
        public async Task<IActionResult> GetPatient(string? name, [FromQuery] Conclusion[] conclusions, PatientSorting? sorting, bool? scheduledVisits = false,
            bool? onlyMine = false, int? page = 1, int? size = 5)
        {
            try
            {
                await _tokenService.CheckToken(HttpContext.Request.Headers["Authorization"].ToString().Substring("Bearer ".Length));
                PatientPagedListModel inspectionId = await _patientSevise.GetPatient(Guid.Parse(User.Identity.Name), name, conclusions, sorting, scheduledVisits, onlyMine, page, size);
